...
const Header = () => {
  ...
  const accommodationLinks = navItems.find(n => n.label === "Accommodation")?.children || [];
  const thingsToDoLinks = navItems.find(n => n.label === "Things to Do")?.children || [];
    ...
    <header className="sticky top-0 z-50 bg-background shadow-sm">
      ...
      <div className="bg-primary text-primary-foreground py-2">
        <div className="container flex justify-between items-center text-sm">
          <a href={`tel:${settings.freeCallPhone?.replace(/\s/g, "")}`} className="flex items-center gap-2 hover:text-accent transition-colors">
            <Phone size={14} />
      ...
      <div className="bg-secondary border-b border-border py-2">
        <div className="container flex flex-wrap justify-center items-center gap-4 sm:gap-8 text-sm">
          ...
          <a href={`tel:${settings.freeCallPhone?.replace(/\s/g, "")}`} className="flex items-center gap-2 text-primary hover:text-accent transition-colors">
            <Phone size={14} />
            <span>Freecall: {settings.freeCallPhone}</span>
          </a>
          <span className="hidden sm:inline text-muted-foreground">|</span>
          <a href={`mailto:${settings.email}`} className="flex items-center gap-2 text-primary hover:text-accent transition-colors">
            <Mail size={14} />
